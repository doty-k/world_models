...
RENDER_MODE = 'semantic' # OPTIONS: 'standard', 'textured' or 'semantic'
...
class CarRacing(gym.Env):
    ...
    def __init__(self):
        ...
        grass_path = os.path.join(dirname, '..', 'textures', 'grass.png')
        road_path = os.path.join(dirname, '..', 'textures', 'road.png')
        self.seed()
        ...
        self.texture = pyglet.image.load(grass_path).get_texture()
        self.road_texture = pyglet.image.load(road_path).get_texture()
    ...
    def render(self, mode='human'):
        ...
        if RENDER_MODE in ['standard', 'textured']:

            self.car.draw(self.viewer, draw_particles = True)
            ...
            if mode=="rgb_array" or mode=="state_pixels":
                ...
                # segmented observation rendering
                win.clear()
                t = self.transform
                if mode=='rgb_array':
                    VP_W = VIDEO_W
                ...
                self.car.render_mode = 'segmented'
                ...
                segment_data = pyglet.image.get_buffer_manager().get_color_buffer().get_image_data()
                seg = np.fromstring(segment_data.data, dtype=np.uint8, sep='')
                seg = seg.reshape(VP_H, VP_W, 4)
                seg = seg[::-1, :, 0:3]

                # textured observation rendering
                ...
                self.render_textured()
                self.car.render_mode = 'standard'
                for w in self.car.wheels:
                	w.color = self.car.hull.color
                for geom in self.viewer.onetime_geoms:
                    geom.render()
                ...
                # standard observation rendering
                win.switch_to()
                win.dispatch_events()
                win.clear()
                ...
                t.disable()
                # disable indicator rendering
                #self.render_indicators(WINDOW_W, WINDOW_H)
                scene_data = pyglet.image.get_buffer_manager().get_color_buffer().get_image_data()
                stan = np.fromstring(scene_data.data, dtype=np.uint8, sep='')
                stan = stan.reshape(VP_H, VP_W, 4)
                stan = stan[::-1, :, 0:3]
        ...
        if mode=='human':
            ...
            t.enable()
            if RENDER_MODE in ['standard', 'semantic']:
                self.render_road()
            elif RENDER_MODE == 'textured':
                self.render_textured()
            for geom in self.viewer.onetime_geoms:
